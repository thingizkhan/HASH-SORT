...
int currentHashSizeIndex = 0; // Index to keep track of the current hash size
    ...
    // const std::size_t bufferSize = 8192; // Define a buffer size, e.g., 8 KB
    // std::vector<char> buffer(bufferSize);
    // std::string line;
    // std::string leftover; // To store incomplete lines at the end of each buffer
...
void getHash() {
    ...
    const std::size_t bufferSize = 16384; // Define a buffer size, e.g., 8 KB
    std::vector<char> buffer(bufferSize);
    std::string line;
    std::string leftover; // To store incomplete lines at the end of each buffer
    ...
    while (file.read(buffer.data(), bufferSize) || file.gcount() > 0) {
        ...
        // Check if there is an incomplete line left in the stream
        if (!stream.eof()) {
            stream >> leftover; // Store the incomplete line to prepend in the next chunk
        }
    }

    // Process the final leftover line if it's not empty
    if (!leftover.empty()) {
        // Extract and process the request from the leftover line here...
    }
